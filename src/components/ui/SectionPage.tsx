...
function AddButton({ label, color, colorLight, onClick }: { label: string; color: string; colorLight: string; onClick?: () => void; }) { const [hov,setHov]=useState(false); return (<div onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{ padding: '0.5rem 1.2rem', borderRadius: '0.85rem', background: hov ? `linear-gradient(135deg, ${colorLight}, ${color})` : color, color: '#fff', fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', boxShadow: hov ? `0 6px 20px -4px ${color}55` : `0 4px 14px -2px ${color}33`, transform: hov ? 'translateY(-2px) scale(1.05)' : 'translateY(0)', }}>{<Plus size={16} />}{label}</div>); }
...
export default function SectionPage({ section, rawMaterials, formulas, products, suppliers, recentProduction, sales = [], invoices = [], customers = [], inventory = [], debts = [], expenses = [], labExperiments = [], academyCourses = [], books = [], formulaEmojis = {} }: SectionPageProps) {
  ...
  const [rawList, setRawList] = useState(rawMaterials);
  const [supplierList, setSupplierList] = useState(suppliers);
  const [formulaList, setFormulaList] = useState(formulas);
  const [productList, setProductList] = useState(products);

  const goTab = (tab: string) => router.push(tab ? `${pathname}?tab=${tab}` : pathname);

  const quickActions = [
    { label: 'إضافة مادة خام', href: `${pathname}?tab=raw`, icon: <FlaskConical size={24} />, color: c, colorLight: cl },
    { label: 'تركيبة جديدة', href: `${pathname}?tab=formula-lab`, icon: <Beaker size={24} />, color: '#7c3aed', colorLight: '#a78bfa' },
    { label: 'بدء تصنيع', href: `${pathname}?tab=production`, icon: <Factory size={24} />, color: '#0891b2', colorLight: '#22d3ee' },
    { label: 'فاتورة جديدة', href: `${pathname}?tab=invoices`, icon: <FileText size={24} />, color: '#ea580c', colorLight: '#f97316' },
  ];
  const statsData = [
    { title: 'المنتجات', value: formatNumber(productList.length, 0), subtitle: 'منتج نهائي', icon: <Package size={20} />, color: c, colorLight: cl },
    { title: 'التركيبات', value: formatNumber(formulaList.length, 0), subtitle: 'تركيبة', icon: <Beaker size={20} />, color: '#7c3aed', colorLight: '#a78bfa' },
    { title: 'المواد الخام', value: formatNumber(rawList.length, 0), subtitle: 'مادة', icon: <FlaskConical size={20} />, color: '#0891b2', colorLight: '#22d3ee' },
    { title: 'الموردين', value: formatNumber(supplierList.length, 0), subtitle: 'مورد', icon: <Truck size={20} />, color: '#d97706', colorLight: '#fbbf24' },
  ];
  const defaultCustomers: CustomerItem[] = [
    { id: '1', name: 'شركة الأمل للتجارة', phone: '01012345678', email: 'info@alamal.com', totalOrders: 24, totalSpent: 185000, lastOrderDate: '2026-01-15', status: 'نشط' },
    { id: '2', name: 'مؤسسة النور', phone: '01123456789', email: 'sales@alnour.com', totalOrders: 18, totalSpent: 95000, lastOrderDate: '2026-01-14', status: 'نشط' },
    { id: '3', name: 'سوبر ماركت السلام', phone: '01234567890', email: 'salam@market.com', totalOrders: 11, totalSpent: 45000, lastOrderDate: '2026-01-12', status: 'نشط' },
    { id: '4', name: 'محلات الوفاء', phone: '01098765432', email: 'wafaa@shop.com', totalOrders: 9, totalSpent: 62000, lastOrderDate: '2026-01-10', status: 'نشط' },
  ];
  const defaultSales: SaleItem[] = productList.slice(0, 4).map((p, i) => {
    const qty = 20 + i * 10;
    const total = p.price * qty;
    const status = i % 3 === 0 ? 'paid' : i % 3 === 1 ? 'partially_paid' : 'confirmed';
    const paid = status === 'paid' ? total : status === 'partially_paid' ? Math.round(total / 2) : 0;
    return {
      id: String(i + 1),
      invoiceNumber: `INV-${i + 1}`,
      customer: defaultCustomers[i % defaultCustomers.length].name,
      products: [{ name: p.name, quantity: qty, price: p.price }],
      total, paid, status, date: `2026-01-${15 - i}`,
    };
  });
  const salesData = sales.length ? sales : defaultSales;
  const salesStats = { todaySales: salesData.reduce((s, x) => s + (x.total || 0), 0) || 15600, weekSales: 85000, monthSales: 485000, todayOrders: salesData.length || 12, avgOrderValue: salesData.length ? Math.round(salesData.reduce((s,x)=>s+x.total,0)/salesData.length) : 1300, };
  ...
  const invoicesData = invoices.length ? invoices : salesData.map(s => ({ id: s.id, number: s.invoiceNumber, customer: s.customer, total: s.total, paid: s.paid, remaining: s.total - s.paid, status: s.status, date: s.date, dueDate: s.date }));
  const customersData = customers.length ? customers : defaultCustomers;
  ...
  const labData: LabItem[] = labExperiments.length ? labExperiments : formulaList.slice(0,4).map((f,i)=>({ id: String(i+1), name: f.name, category: f.category, status: i===0?'مكتمل':i===1?'قيد التجربة':'مخطط', progress: i===0?100:i===1?65:20, ingredients: f.ingredients, result: i===0?'ناجح':i===1?'قيد الاختبار':'-', date: '2026-01-15' }));
  ...
  const filteredRaw = q ? rawList.filter(r => r.name.toLowerCase().includes(q) || r.status.includes(query)) : rawList;
  const filteredSuppliers = q ? supplierList.filter(s => s.name.toLowerCase().includes(q) || s.material.toLowerCase().includes(q)) : supplierList;
  const filteredFormulas = q ? formulaList.filter(f => f.name.toLowerCase().includes(q) || f.category.toLowerCase().includes(q)) : formulaList;
  const filteredProducts = q ? productList.filter(pr => pr.name.toLowerCase().includes(q) || pr.category.toLowerCase().includes(q)) : productList;
  const filteredSales = q ? salesData.filter(s => s.customer.toLowerCase().includes(q) || s.invoiceNumber.toLowerCase().includes(q)) : salesData;
  const filteredInvoices = q ? invoicesData.filter(inv => inv.customer.toLowerCase().includes(q) || inv.number.toLowerCase().includes(q)) : invoicesData;
  const filteredCustomers = q ? customersData.filter(cust => cust.name.toLowerCase().includes(q) || cust.phone.includes(query)) : customersData;

  const handleAdd = () => {
    if (!addName.trim()) return;
    if (activeTab === 'raw') setRawList(prev => [...prev, { name: addName.trim(), stock: 0, unit: 'كجم', min: 10, status: 'منخفض' }]);
    if (activeTab === 'suppliers') setSupplierList(prev => [...prev, { name: addName.trim(), material: section.name, phone: '01000000000' }]);
    if (activeTab === 'formulas') setFormulaList(prev => [...prev, { name: addName.trim(), status: 'مسودة', ingredients: 0, category: section.name }]);
    if (activeTab === 'products') setProductList(prev => [...prev, { name: addName.trim(), price: 0, stock: 0, category: section.name }]);
    setAddName('');
    setShowAdd(false);
  };
...
<div className="card animate-slide-up" style={{ animationDelay: '0.5s', animationFillMode: 'both', border: '2px solid #e2e8f0', background: 'rgba(255,255,255,0.7)' }}><div className="card-header"><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: c }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><ShoppingCart size={24} /></span>إجراءات سريعة</h2></div><div className="card-body"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '1.25rem' }}>{quickActions.map((action, index) => (<QuickActionLink key={action.label} action={action} index={index} sectionColor={c} />))}</div></div></div>
          <div className="two-col-layout">
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.7s', border: `2px solid #7c3aed30`, borderTop: `4px solid #7c3aed` }}><div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, #7c3aed08, rgba(248,250,252,0.9))` }}><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#7c3aed' }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, #a78bfa, #7c3aed)` }}><Beaker size={24} /></span>آخر التركيبات - {section.name}</h2><SectionLink label="عرض الكل" color="#7c3aed" onClick={() => goTab('formulas')} /></div><div className="card-body p-0">{formulaList.slice(0, 4).map((f, i) => (<ListItem key={i} name={f.name} subtitle={`${f.category} • ${f.ingredients} مكون`} badge={f.status} emoji={formulaEmojis[f.name] || '🧪'} color="#7c3aed" />))}</div></div>
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.8s', border: `2px solid #f9731630`, borderTop: `4px solid #f97316` }}><div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, #f9731608, rgba(248,250,252,0.9))` }}><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#f97316' }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, #fb923c, #f97316)` }}><Package size={24} /></span>آخر المنتجات - {section.name}</h2><SectionLink label="عرض الكل" color="#f97316" onClick={() => goTab('products')} /></div><div className="card-body p-0">{productList.slice(0, 4).map((p, i) => (<ListItem key={i} name={p.name} subtitle={`${p.category} • مخزون: ${p.stock}`} value={formatCurrency(p.price)} emoji="📦" color="#f97316" />))}</div></div>
          </div>
          ...
          <div className="two-col-layout">
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.85s', border: `2px solid #10b98130`, borderTop: `4px solid #10b981` }}>
              <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, #10b98108, rgba(248,250,252,0.9))` }}>
                ...
                <SectionLink label="عرض الكل" color="#10b981" onClick={() => goTab('sales')} />
              </div>
            ...
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.9s', border: `2px solid #0891b230`, borderTop: `4px solid #0891b2` }}>
              <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, #0891b208, rgba(248,250,252,0.9))` }}>
                ...
                <SectionLink label="عرض الكل" color="#0891b2" onClick={() => goTab('production')} />
              </div>
              <div className="card-body p-0">
                {recentProduction.slice(0,4).map((b,i)=>(
                  <ListItem key={i} name={b.product} subtitle={`${b.qty} • ${b.progress}%`} badge={b.status} emoji={String(i + 1)} color="#0891b2" />
                ))}
              </div>
            </div>
          </div>
          {rawList.filter(r => r.status === 'منخفض').length > 0 && (
            <div className="card animate-slide-up section-color-red" style={{ animationDelay: '1.1s', animationFillMode: 'both', border: '2px solid rgba(220,38,38,0.3)', borderTop: '4px solid #dc2626' }}>
              <div className="card-header flex items-center justify-between" style={{ background: 'linear-gradient(to bottom, rgba(220,38,38,0.08), rgba(248,250,252,0.9))' }}>
                ...
                <SectionLink label="إدارة المخزون" color="#dc2626" onClick={() => goTab('inventory')} />
              </div>
              <div className="card-body">
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.25rem' }}>
                  {rawList.filter(r => r.status === 'منخفض').map((item, index) => (
                    <div
                      key={index}
          ...
          <div className="card" style={{ border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${c}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: c }}>
                ...
                المواد الخام ({rawList.length}) - {section.name}
              </h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <div onClick={()=> alert(`تصدير مواد خام - ${section.name}: ${rawList.length} مادة`)} style={{ padding: '0.5rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}25`, background: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem', transition: 'all 0.2s' }} onMouseEnter={(e)=>{e.currentTarget.style.background=tColor; e.currentTarget.style.color='#fff'}} onMouseLeave={(e)=>{e.currentTarget.style.background='#fff'; e.currentTarget.style.color='#000'}}>📤 تصدير</div>
                <AddButton label="إضافة مادة" color={tColor} colorLight={tLight} onClick={()=>setShowAdd(true)} />
              </div>
            </div>
            <div className="card-body">
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <div style={{ position: 'relative', flex: 1 }}>
                  ...
                  <input placeholder="🔍 ابحث بالاسم، الكود، الاسم العلمي..." value={query} onChange={(e)=>setQuery(e.target.value)} style={{ width: '100%', padding: '0.75rem 2.5rem 0.75rem 1rem', borderRadius: '0.85rem', border: `1.5px solid ${tColor}20`, fontWeight: 600, outline: 'none' }} />
                </div>
      ...
      {activeTab === 'suppliers' && (
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${c}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: c, fontSize: '0.85rem' }}>إجمالي الموردين</p><p className="stat-value">{supplierList.length}</p></div><div className="stat-icon" style={{ background: `${c}15`, color: c }}><Truck size={20} /></div></div></div>
            <div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">إجمالي المشتريات</p><p className="stat-value">{formatCurrency(supplierList.length * 120000)}</p></div><div className="stat-icon bg-blue-100 text-blue-600"><Package size={20} /></div></div></div>
            ...
            <div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">المواد</p><p className="stat-value">{supplierList.length * 8}</p></div><div className="stat-icon bg-purple-100 text-purple-600"><FlaskConical size={20} /></div></div></div>
          </div>
          <div className="card" style={{ border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${c}08, rgba(248,250,252,0.9))` }}>
      ...
      {activeTab === 'formulas' && (
        <div className="space-y-6">
          <div className="card" style={{ border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${c}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: c, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><Beaker size={20} /></span>التركيبات ({formulaList.length}) - {section.name}</h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}>
                  <Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  <input placeholder="ابحث في التركيبات..." value={query} onChange={(e)=>setQuery(e.target.value)} style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${c}20`, fontWeight: 600, outline: 'none' }} />
                </div>
                <AddButton label="تركيبة جديدة" color={c} colorLight={cl} onClick={()=>setShowAdd(true)} />
              </div>
            </div>
            ...
            <div className="card-body p-0">{filteredFormulas.map((f, i) => (<ListItem key={i} name={f.name} subtitle={`${f.category} • ${f.ingredients} مكون • ${section.name}`} badge={f.status} emoji={formulaEmojis[f.name] || '🧪'} color={c} />))}</div>
            <div className="card-footer" style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem 1.25rem', background: '#f8fafc', borderTop: `1px solid ${c}10` }}><span style={{ fontWeight: 700, color: '#64748b', fontSize: '0.85rem' }}>إجمالي {formulaList.length} تركيبة في {section.name}</span><div style={{ display: 'flex', gap: '0.5rem' }}><span style={{ padding: '0.3rem 0.7rem', background: '#f0fdf4', color: '#16a34a', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem' }}>✓ {formulaList.filter(f=>f.status==='معتمدة').length} معتمدة</span><span style={{ padding: '0.3rem 0.7rem', background: '#eff6ff', color: '#2563eb', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem' }}>🧪 {formulaList.filter(f=>f.status==='تحت الاختبار').length} اختبار</span></div></div>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="card" style={{ border: `2px solid ${c}20`, borderTop: `3px solid ${c}`, gridColumn: 'span 2' }}>
              ...
              <div className="card-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>اسم التركيبة</label><input placeholder={`مثال: ${formulaList[0]?.name || 'تركيبة جديدة'}`} defaultValue={formulaList[0]?.name} style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${c}20`, fontWeight: 700, outline: 'none' }} /></div>
                <div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>التصنيف - {section.name}</label><select style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${c}20`, fontWeight: 700 }}><option>{formulaList[0]?.category || section.name}</option><option>{section.name} - مبتكر</option></select></div>
      ...
      {activeTab === 'products' && (
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${c}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: c, fontSize: '0.85rem' }}>إجمالي المنتجات</p><p className="stat-value">{productList.length}</p></div><div className="stat-icon" style={{ background: `${c}15`, color: c }}><Package size={20} /></div></div></div>
            <div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">قيمة المخزون</p><p className="stat-value">{formatCurrency(productList.reduce((s,p)=>s+p.price*p.stock,0))}</p></div><div className="stat-icon bg-green-100 text-green-600"><DollarSign size={20} /></div></div></div>
            <div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">مخزون منخفض</p><p className="stat-value text-amber-600">{productList.filter(p=>p.stock<150).length}</p></div><div className="stat-icon bg-amber-100 text-amber-600"><AlertTriangle size={20} /></div></div></div>
            <div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">متاح</p><p className="stat-value text-green-600">{productList.filter(p=>p.stock>=150).length}</p></div><div className="stat-icon bg-green-100 text-green-600"><CheckCircle size={20} /></div></div></div>
          </div>
          <div className="card" style={{ border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${c}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: c, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><Package size={20} /></span>المنتجات ({productList.length}) - {section.name}</h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}><Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} /><input placeholder="ابحث في المنتجات..." value={query} onChange={(e)=>setQuery(e.target.value)} style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${c}20`, fontWeight: 600 }} /></div>
                <AddButton label="منتج جديد" color={c} colorLight={cl} onClick={()=>setShowAdd(true)} />
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="card" style={{ border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${c}08, rgba(248,250,252,0.9))` }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}><Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} /><input placeholder="ابحث برقم الدفعة أو المنتج..." value={query} onChange={(e)=>setQuery(e.target.value)} style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${c}20`, fontWeight: 600 }} /></div>
                <AddButton label="بدء تصنيع" color={c} colorLight={cl} onClick={()=>setShowAdd(true)} />
              </div>
            </div>
      ...
      {showAdd && (
        <div onClick={()=>setShowAdd(false)} style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.45)', zIndex:80, display:'flex', alignItems:'center', justifyContent:'center', padding:'1.5rem' }}>
          <div onClick={(e)=>e.stopPropagation()} style={{ width:'min(520px,100%)', background:'#fff', borderRadius:'1.25rem', padding:'1.5rem', boxShadow:'0 25px 60px -20px rgba(15,23,42,0.45)', border:`2px solid ${c}25` }}>
            <h3 style={{ margin:0, fontWeight:900, fontSize:'1.25rem', color:c }}>إضافة {activeTab === 'raw' ? 'مادة خام' : activeTab === 'suppliers' ? 'مورد' : activeTab === 'formulas' ? 'تركيبة' : activeTab === 'products' ? 'منتج' : 'عنصر'} — {section.name}</h3>
            <p style={{ margin:'0.4rem 0 1rem', color:'#64748b', fontWeight:600 }}>اكتب الاسم ثم احفظ. العنصر هيظهر فوراً في قائمة القسم.</p>
            <input autoFocus value={addName} onChange={(e)=>setAddName(e.target.value)} placeholder="الاسم" onKeyDown={(e)=>{ if(e.key==='Enter') handleAdd(); }} style={{ width:'100%', padding:'0.8rem 1rem', borderRadius:'0.85rem', border:`1.5px solid ${c}30`, fontWeight:700, outline:'none' }} />
            <div style={{ display:'flex', gap:'0.6rem', justifyContent:'flex-end', marginTop:'1.1rem' }}>
              <button type="button" onClick={()=>setShowAdd(false)} style={{ padding:'0.6rem 1.1rem', borderRadius:'0.75rem', border:'1.5px solid #e2e8f0', background:'#fff', fontWeight:800, cursor:'pointer' }}>إلغاء</button>
              <button type="button" onClick={handleAdd} style={{ padding:'0.6rem 1.2rem', borderRadius:'0.75rem', border:'none', background:`linear-gradient(135deg, ${cl}, ${c})`, color:'#fff', fontWeight:800, cursor:'pointer' }}>حفظ</button>
            </div>
          </div>
        </div>
      )}
