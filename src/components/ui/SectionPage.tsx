...
<div className="card animate-slide-up" style={{ animationDelay: '0.5s', animationFillMode: 'both', border: '2px solid #e2e8f0', background: 'rgba(255,255,255,0.7)' }}><div className="card-header"><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: c }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><ShoppingCart size={24} /></span>إجراءات سريعة</h2></div><div className="card-body"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '1.25rem' }}>{quickActions.map((action, index) => (<QuickActionLink key={action.label} action={action} index={index} sectionColor={c} />))}</div></div></div>
            ...
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.85s', border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
              ...
              <div className="card-body p-0">
                {topProducts.slice(0,4).map((p,i)=>(
                  <ListItem key={i} name={p.name} subtitle={`${p.quantity} وحدة مباعة`} value={formatCurrency(p.quantity * 25)} emoji={p.emoji} color={c} />
                ))}
              </div>
            </div>
            ...
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.9s', border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}>
              ...
              <div className="card-body p-0">
                {recentProduction.slice(0,4).map((b,i)=>(
                  <ListItem key={i} name={b.product} subtitle={`${b.qty} • ${b.progress}%`} badge={b.status} emoji={b.batch.split('-').pop() || '🏭'} color={c} />
                ))}
              </div>
            </div>
          </div>
      ...
      {activeTab === 'raw' && (
        <div className="space-y-6">
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: tColor }}>
                <span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}>
                  <FlaskConical size={24} />
                </span>
                المواد الخام ({rawMaterials.length}) - {section.name}
              </h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ padding: '0.5rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}25`, background: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>📥 استيراد</div>
                <div style={{ padding: '0.5rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}25`, background: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>📤 تصدير</div>
            ...
            <div className="card-body">
              <div style={{ display: 'flex', gap: '0.75rem' }}>
                <div style={{ position: 'relative', flex: 1 }}>
                  ...
                  <input placeholder="🔍 ابحث بالاسم، الكود، الاسم العلمي..." style={{ width: '100%', padding: '0.75rem 2.5rem 0.75rem 1rem', borderRadius: '0.85rem', border: `1.5px solid ${tColor}20`, fontWeight: 600, outline: 'none' }} />
                </div>
                <div style={{ padding: '0.75rem 1.2rem', borderRadius: '0.85rem', border: `1.5px solid ${tColor}20`, background: '#fff', fontWeight: 700, cursor: 'pointer' }}>🔧 فلترة</div>
              </div>
            </div>
            <div className="card-body p-0">
              <table className="table">
                <thead>
                  <tr>
                    <th style={{ color: tColor, fontWeight: 900 }}>الكود</th>
                    <th style={{ color: tColor, fontWeight: 900 }}>اسم المادة</th>
                    <th style={{ color: tColor, fontWeight: 900 }}>التصنيف</th>
                    <th style={{ color: tColor, fontWeight: 900 }}>السعر</th>
                    <th style={{ color: tColor, fontWeight: 900 }}>الكمية</th>
                    <th style={{ color: tColor, fontWeight: 900 }}>الحالة</th>
                    <th style={{ color: tColor, fontWeight: 900 }}>الإجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {rawMaterials.map((r, i) => (
                    <TableRowWithHover key={i} color={c}>
                      <td style={{ fontFamily: 'monospace', fontWeight: 800, fontSize: '0.85rem', color: tColor }}>RM-{String(i+1).padStart(3,'0')}</td>
                      <td>
                        <div>
                      ...
                      <td><span style={{ padding: '0.25rem 0.6rem', borderRadius: '0.5rem', background: `${tColor}12`, color: tColor, fontWeight: 700, fontSize: '0.75rem' }}>{section.name}</span></td>
      ...
      {activeTab === 'suppliers' && (
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>إجمالي الموردين</p><p className="stat-value">{suppliers.length}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><Truck size={20} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Truck size={20} /></span>الموردون - {section.name}</h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}>
                  <Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  <input placeholder="ابحث عن مورد..." style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 600, outline: 'none' }} />
                </div>
                <AddButton label="إضافة مورد" color={c} colorLight={cl} />
              </div>
            </div>
            <div className="card-body"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '1.25rem' }}>{suppliers.map((s,i)=>(<div key={i} className="card hover:shadow-xl transition-all" style={{ border: `1.5px solid ${tColor}15`, background: '#fff' }}><div className="card-body"><div style={{ display: 'flex', gap: '1rem' }}><div style={{ width: '4rem', height: '4rem', borderRadius: '1rem', background: `linear-gradient(135deg, ${tColor}15, ${tLight}15)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>🏭</div><div style={{ flex: 1 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}><h3 style={{ fontWeight: 900, color: '#1e293b', margin: 0 }}>{s.name}</h3><span style={{ padding: '0.2rem 0.6rem', background: `${tColor}12`, color: tColor, borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.7rem' }}>{s.material}</span></div><p style={{ fontWeight: 600, fontSize: '0.8rem', color: '#64748b', margin: '0 0 0.75rem 0', display: 'flex', alignItems: 'center', gap: '0.3rem' }}><Phone size={12} /> {s.phone} • {section.name}</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', fontSize: '0.8rem' }}><div><p style={{ color: '#94a3b8', fontWeight: 600, margin: 0 }}>المواد</p><p style={{ fontWeight: 800, margin: 0 }}>{8 + i*2}</p></div><div><p style={{ color: '#94a3b8', fontWeight: 600, margin: 0 }}>المشتريات</p><p style={{ fontWeight: 800, margin: 0 }}>{formatCurrency(50000 + i*15000)}</p></div><div><p style={{ color: '#94a3b8', fontWeight: 600, margin: 0 }}>المستحق</p><p style={{ fontWeight: 800, color: i%2?'#dc2626':'#16a34a', margin: 0 }}>{formatCurrency(i%2?8000:0)}</p></div></div><div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', paddingTop: '1rem', borderTop: `1px solid ${tColor}10` }}><div style={{ flex: 1, padding: '0.5rem', borderRadius: '0.6rem', border: `1.5px solid ${tColor}20`, textAlign: 'center', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}>👁️ عرض التفاصيل</div><div style={{ flex: 1, padding: '0.5rem', borderRadius: '0.6rem', background: c, color: '#fff', textAlign: 'center', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer' }}>📦 المواد الخام</div></div></div></div></div></div>))}</div></div>
          </div>
        </div>
      )}
      {activeTab === 'formulas' && (
        <div className="space-y-6">
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Beaker size={20} /></span>التركيبات ({formulas.length}) - {section.name}</h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}>
                  <Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} />
                  <input placeholder="ابحث في التركيبات..." style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 600, outline: 'none' }} />
                </div>
            ...
            <div className="card-footer" style={{ display: 'flex', justifyContent: 'space-between', padding: '1rem 1.25rem', background: '#f8fafc', borderTop: `1px solid ${tColor}10` }}><span style={{ fontWeight: 700, color: '#64748b', fontSize: '0.85rem' }}>إجمالي {formulas.length} تركيبة في {section.name}</span><div style={{ display: 'flex', gap: '0.5rem' }}><span style={{ padding: '0.3rem 0.7rem', background: '#f0fdf4', color: '#16a34a', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem' }}>✓ {formulas.filter(f=>f.status==='معتمدة').length} معتمدة</span><span style={{ padding: '0.3rem 0.7rem', background: '#eff6ff', color: '#2563eb', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem' }}>🧪 {formulas.filter(f=>f.status==='تحت الاختبار').length} اختبار</span></div></div>
          </div>
        </div>
      )}
      {activeTab === 'formula-lab' && (
        <div className="space-y-6">
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}>
              <div>
                <h2 style={{ fontWeight: 900, fontSize: '1.5rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem', margin: 0 }}>
                  <span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}>
                    <Sparkles size={24} />
                  </span>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ padding: '0.6rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, background: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}>📋 نسخ التركيبة</div>
                <div style={{ padding: '0.6rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, background: '#fff', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer' }}>📄 تصدير PDF</div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="card" style={{ border: `2px solid ${tColor}20`, borderTop: `3px solid ${tColor}`, gridColumn: 'span 2' }}>
              <div className="card-header" style={{ background: `${tColor}08` }}><h3 style={{ fontWeight: 900, color: tColor, margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Beaker size={18} /> معلومات التركيبة</h3></div>
              <div className="card-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>اسم التركيبة</label><input placeholder={`مثال: ${formulas[0]?.name || 'تركيبة جديدة'}`} defaultValue={formulas[0]?.name} style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 700, outline: 'none' }} /></div>
                <div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>التصنيف - {section.name}</label><select style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 700 }}><option>{formulas[0]?.category || section.name}</option><option>{section.name} - مبتكر</option></select></div>
                <div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>حجم الدفعة</label><div style={{ display: 'flex', gap: '0.5rem' }}><input type="number" defaultValue={100} style={{ flex: 1, padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 700 }} /><select style={{ padding: '0.7rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 700 }}><option>كجم</option><option>لتر</option><option>جم</option></select></div></div>
                <div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>تاريخ الإنشاء</label><input type="date" defaultValue="2026-01-15" style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 700 }} /></div>
              </div>
            </div>
            <div className="card" style={{ border: `2px solid ${tColor}20`, borderTop: `3px solid ${tColor}` }}>
              <div className="card-header" style={{ background: `${tColor}08` }}><h3 style={{ fontWeight: 900, color: tColor, margin: 0 }}>ملخص سريع</h3></div>
              <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: `${tColor}08`, borderRadius: '0.75rem' }}><span style={{ fontWeight: 700, color: '#475569', fontSize: '0.85rem' }}>التجارب النشطة</span><span style={{ fontWeight: 900, color: tColor }}>{labData.filter(x=>x.status==='قيد التجربة').length}</span></div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="card" style={{ border: `2px solid ${tColor}20`, borderTop: `3px solid ${tColor}`, gridColumn: 'span 2' }}>
              <div className="card-header flex items-center justify-between" style={{ background: `${tColor}08` }}><h3 style={{ fontWeight: 900, color: tColor, margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Layers size={18} /> المكونات - {section.name}</h3><AddButton label="إضافة مكون" color={c} colorLight={cl} /></div>
              <div className="card-body p-0">
                <table className="table">
                  <thead><tr><th style={{ color: tColor }}>المادة</th><th style={{ color: tColor }}>النسبة %</th><th style={{ color: tColor }}>الكمية</th><th style={{ color: tColor }}>التكلفة</th><th style={{ color: tColor }}>إجراء</th></tr></thead>
                  <tbody>
                    {rawMaterials.slice(0,3).map((r,i)=>(
                      <tr key={i} style={{ borderBottom: `1px solid ${tColor}10` }}>
                        <td><div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}><div style={{ width: '2rem', height: '2rem', borderRadius: '0.5rem', background: `${tColor}15`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>🧪</div><span style={{ fontWeight: 700 }}>{r.name}</span></div></td>
                        <td><input type="number" defaultValue={10 + i*5} style={{ width: '60px', padding: '0.4rem', borderRadius: '0.5rem', border: `1px solid ${tColor}20`, fontWeight: 700, textAlign: 'center' }} /> %</td>
                        <td style={{ fontWeight: 700 }}>{(10 + i*5)} {r.unit}</td>
                        <td style={{ fontWeight: 800, color: tColor }}>{formatCurrency((10 + i*5)*(5 + i))}</td>
                ...
                <div style={{ padding: '1rem', textAlign: 'center' }}><div style={{ display: 'inline-flex', padding: '0.6rem 1.2rem', borderRadius: '0.75rem', border: `1.5px dashed ${tColor}40`, color: tColor, fontWeight: 800, cursor: 'pointer', gap: '0.5rem', alignItems: 'center' }}><Plus size={16} /> إضافة مكون</div></div>
              </div>
            </div>

            <div className="space-y-4">
              <div className="card" style={{ border: `2px solid ${tColor}20`, borderTop: `3px solid ${tColor}` }}>
                <div className="card-header" style={{ background: `${tColor}08` }}><h3 style={{ fontWeight: 900, color: tColor, margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Calculator size={18} /> حساب التكلفة</h3></div>
                <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                  <div><label style={{ fontWeight: 700, fontSize: '0.8rem', color: '#475569' }}>تكلفة المواد الخام</label><div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.25rem' }}><input defaultValue="0.00" style={{ flex: 1, padding: '0.5rem 0.75rem', borderRadius: '0.6rem', border: `1.5px solid ${tColor}15`, fontWeight: 700 }} /><span style={{ fontWeight: 700, color: '#64748b' }}>جنيه</span></div></div>
                  <div><label style={{ fontWeight: 700, fontSize: '0.8rem', color: '#475569' }}>تكلفة التعبئة والتغليف</label><input type="number" defaultValue={0} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.6rem', border: `1.5px solid ${tColor}15`, fontWeight: 700, marginTop: '0.25rem' }} /></div>
                  <div><label style={{ fontWeight: 700, fontSize: '0.8rem', color: '#475569' }}>تكلفة العمالة</label><input type="number" defaultValue={0} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.6rem', border: `1.5px solid ${tColor}15`, fontWeight: 700, marginTop: '0.25rem' }} /></div>
                  <div><label style={{ fontWeight: 700, fontSize: '0.8rem', color: '#475569' }}>تكلفة الكهرباء</label><input type="number" defaultValue={0} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.6rem', border: `1.5px solid ${tColor}15`, fontWeight: 700, marginTop: '0.25rem' }} /></div>
                  <div><label style={{ fontWeight: 700, fontSize: '0.8rem', color: '#475569' }}>المصروفات العامة</label><input type="number" defaultValue={0} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.6rem', border: `1.5px solid ${tColor}15`, fontWeight: 700, marginTop: '0.25rem' }} /></div>
              ...
              <div className="card" style={{ border: `2px solid #10b98130`, borderTop: `3px solid #10b981` }}>
                ...
                <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontWeight: 900, color: '#1e293b' }}>إجمالي التكلفة</span><span style={{ fontWeight: 900, color: tColor, fontSize: '1.1rem' }}>0.00 جنيه</span></div>
      ...
      {activeTab === 'products' && (
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>إجمالي المنتجات</p><p className="stat-value">{products.length}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><Package size={20} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Package size={20} /></span>المنتجات ({products.length}) - {section.name}</h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}><Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} /><input placeholder="ابحث في المنتجات..." style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 600 }} /></div>
            ...
            <div className="card-body p-0">
              <table className="table">
                <thead><tr><th style={{ color: tColor }}>المنتج</th><th style={{ color: tColor }}>التصنيف</th><th style={{ color: tColor }}>السعر</th><th style={{ color: tColor }}>المخزون</th><th style={{ color: tColor }}>القيمة</th><th style={{ color: tColor }}>الحالة</th><th style={{ color: tColor }}>إجراءات</th></tr></thead>
                <tbody>{products.map((p, i) => (<TableRowWithHover key={i} color={c}><td><div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span style={{ fontSize: '1.5rem' }}>📦</span><div><p style={{ fontWeight: 800, margin: 0 }}>{p.name}</p><p style={{ fontWeight: 600, fontSize: '0.7rem', color: '#94a3b8', margin: 0 }}>{section.name}</p></div></div></td><td><span style={{ padding: '0.25rem 0.6rem', background: `${tColor}12`, color: tColor, borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem' }}>{p.category}</span></td><td style={{ fontWeight: 800, color: tColor }}>{formatCurrency(p.price)}</td><td style={{ fontWeight: 700, color: p.stock < 150 ? '#d97706' : '#15803d' }}>{p.stock}</td><td style={{ fontWeight: 700 }}>{formatCurrency(p.price*p.stock)}</td><td><StatusBadge status={p.stock < 100 ? 'منخفض' : 'متوفر'} /></td><td><div style={{ display: 'flex', gap: '0.25rem' }}><span style={{ width: '28px', height: '28px', borderRadius: '0.5rem', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>👁️</span><span style={{ width: '28px', height: '28px', borderRadius: '0.5rem', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>✏️</span></div></td></TableRowWithHover>))}</tbody>
              </table>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'production' && (
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>إجمالي الدفعات</p><p className="stat-value">{recentProduction.length}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><Factory size={20} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}>
            <div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}>
              <h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Factory size={20} /></span>التصنيع - {section.name} ({recentProduction.length})</h2>
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                <div style={{ position: 'relative' }}><Search size={16} style={{ position: 'absolute', right: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: '#94a3b8' }} /><input placeholder="ابحث برقم الدفعة أو المنتج..." style={{ padding: '0.6rem 2rem 0.6rem 0.8rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 600 }} /></div>
            ...
            <div className="card-body p-0">
              <table className="table">
                <thead><tr><th style={{ color: tColor }}>رقم الدفعة</th><th style={{ color: tColor }}>المنتج</th><th style={{ color: tColor }}>الكمية</th><th style={{ color: tColor }}>التقدم</th><th style={{ color: tColor }}>الحالة</th><th style={{ color: tColor }}>المسؤول</th><th style={{ color: tColor }}>إجراءات</th></tr></thead>
                <tbody>{recentProduction.map((b, i) => (<TableRowWithHover key={i} color={c}><td style={{ fontWeight: 800, fontFamily: 'monospace', fontSize: '0.85rem', color: tColor }}>{b.batch}</td><td><div><p style={{ fontWeight: 800, margin: 0 }}>{b.product}</p><p style={{ fontWeight: 600, fontSize: '0.7rem', color: '#94a3b8', margin: 0 }}>v2.1 • {section.name}</p></div></td><td style={{ fontWeight: 700 }}>{b.qty}</td><td><ProgressBar progress={b.progress} color={c} colorLight={cl} /></td><td><StatusBadge status={b.status} /></td><td style={{ fontWeight: 600, fontSize: '0.85rem' }}>أحمد</td><td><div style={{ display: 'flex', gap: '0.25rem' }}><span style={{ width: '28px', height: '28px', borderRadius: '0.5rem', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>👁️</span><span style={{ width: '28px', height: '28px', borderRadius: '0.5rem', background: '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>✏️</span></div></td></TableRowWithHover>))}</tbody>
              </table>
            </div>
          </div>
        </div>
      )}
      {activeTab === 'inventory' && (<div className="space-y-6"><div className="grid grid-cols-4 gap-4"><div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>قيمة المخزون</p><p className="stat-value" style={{ color: tColor }}>{formatCurrency(inventoryValue)}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><Warehouse size={22} /></div></div></div><div className="stat-card" style={{ borderTop: '3px solid #dc2626' }}><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-red-700">منخفض</p><p className="stat-value text-red-700">{lowStockCount}</p></div><div className="stat-icon bg-red-100 text-red-600"><AlertTriangle size={22} /></div></div></div><div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">خام</p><p className="stat-value">{formatCurrency(rawValue)}</p></div><div className="stat-icon bg-blue-100 text-blue-600"><FlaskConical size={22} /></div></div></div><div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">منتجات</p><p className="stat-value">{formatCurrency(prodValue)}</p></div><div className="stat-icon bg-green-100 text-green-600"><Package size={22} /></div></div></div></div><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Warehouse size={20} /></span>المخزون - {section.name}</h2><AddButton label="إضافة" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: tColor }}>الكود</th><th style={{ color: tColor }}>الاسم</th><th style={{ color: tColor }}>النوع</th><th style={{ color: tColor }}>المتاح</th><th style={{ color: tColor }}>الوحدة</th><th style={{ color: tColor }}>القيمة</th><th style={{ color: tColor }}>الحالة</th></tr></thead><tbody>{inventoryData.map(item=>(<TableRowWithHover key={item.id} color={c}><td style={{ fontFamily: 'monospace', fontWeight: 800, color: tColor }}>{item.code}</td><td style={{ fontWeight: 700 }}>{item.name}</td><td><span style={{ padding: '0.2rem 0.6rem', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem', background: item.type==='raw'?'#dbeafe':'#dcfce7', color: item.type==='raw'?'#1d4ed8':'#15803d' }}>{item.type==='raw'?'خام':'منتج'}</span></td><td style={{ fontWeight: 800 }}>{item.current}</td><td>{item.unit}</td><td style={{ fontWeight: 800, color: tColor }}>{formatCurrency(item.totalValue)}</td><td><StatusBadge status={item.status} /></td></TableRowWithHover>))}</tbody></table></div></div></div>)}
      {activeTab === 'sales' && (<div className="space-y-6"><div className="grid grid-cols-4 gap-4"><div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ color: tColor, fontWeight: 800, fontSize: '0.85rem' }}>مبيعات اليوم</p><p className="stat-value" style={{ color: tColor }}>{formatCurrency(salesStats.todaySales)}</p></div><div className="stat-icon" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})`, color: '#fff' }}><TrendingUp size={22} /></div></div></div><div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">أسبوع</p><p className="stat-value">{formatCurrency(salesStats.weekSales)}</p></div><div className="stat-icon bg-blue-100 text-blue-600"><Calendar size={22} /></div></div></div><div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">طلبات</p><p className="stat-value">{salesStats.todayOrders}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><ShoppingCart size={22} /></div></div></div><div className="stat-card"><div className="flex items-center justify-between"><div><p className="text-sm font-bold">متوسط</p><p className="stat-value">{formatCurrency(salesStats.avgOrderValue)}</p></div><div className="stat-icon bg-amber-100 text-amber-600"><DollarSign size={22} /></div></div></div></div><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><ShoppingCart size={20} /></span>المبيعات - {section.name}</h2><AddButton label="بيع جديد" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: tColor }}>الفاتورة</th><th style={{ color: tColor }}>العميل</th><th style={{ color: tColor }}>المنتجات</th><th style={{ color: tColor }}>الإجمالي</th><th style={{ color: tColor }}>المدفوع</th><th style={{ color: tColor }}>الحالة</th><th style={{ color: tColor }}>التاريخ</th></tr></thead><tbody>{(sales.length?sales:[]).map(s=>(<TableRowWithHover key={s.id} color={c}><td style={{ fontWeight: 800, fontFamily: 'monospace', color: tColor }}>{s.invoiceNumber}</td><td style={{ fontWeight: 700 }}>{s.customer}</td><td><div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap' }}>{s.products.slice(0,2).map((p,i)=>(<span key={i} style={{ fontSize: '0.75rem', background: `${tColor}12`, color: tColor, padding: '0.2rem 0.5rem', borderRadius: '0.5rem', fontWeight: 700 }}>{p.name.split(' ')[0]} ({p.quantity})</span>))}</div></td><td style={{ fontWeight: 800 }}>{formatCurrency(s.total)}</td><td style={{ color: '#15803d', fontWeight: 800 }}>{formatCurrency(s.paid)}</td><td><StatusBadge status={s.status==='paid'?'تم الدفع':s.status==='partially_paid'?'مدفوع جزئياً':'مؤكد'} /></td><td>{s.date}</td></TableRowWithHover>))}</tbody></table></div></div></div>)}
      {activeTab === 'invoices' && (<div className="space-y-6"><div className="grid grid-cols-4 gap-4"><div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ color: tColor, fontWeight: 800, fontSize: '0.85rem' }}>إجمالي الفواتير</p><p className="stat-value">{invoicesData.length}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><FileText size={22} /></div></div></div><div className="stat-card" style={{ borderTop: '3px solid #16a34a' }}><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-green-700">مدفوعة</p><p className="stat-value text-green-700">{invoicesData.filter(i=>i.status==='paid').length}</p></div><div className="stat-icon bg-green-100 text-green-600"><CheckCircle size={22} /></div></div></div><div className="stat-card" style={{ borderTop: '3px solid #d97706' }}><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-amber-700">جزئياً</p><p className="stat-value text-amber-700">{invoicesData.filter(i=>i.status==='partially_paid').length}</p></div><div className="stat-icon bg-amber-100 text-amber-600"><Clock size={22} /></div></div></div><div className="stat-card" style={{ borderTop: '3px solid #dc2626' }}><div className="flex items-center justify-between"><div><p className="text-sm font-bold text-red-700">غير مدفوعة</p><p className="stat-value text-red-700">{invoicesData.filter(i=>i.status==='confirmed').length}</p></div><div className="stat-icon bg-red-100 text-red-600"><XCircle size={22} /></div></div></div></div><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><FileText size={20} /></span>الفواتير - {section.name}</h2><AddButton label="فاتورة جديدة" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: tColor }}>رقم الفاتورة</th><th style={{ color: tColor }}>العميل</th><th style={{ color: tColor }}>الإجمالي</th><th style={{ color: tColor }}>المدفوع</th><th style={{ color: tColor }}>المتبقي</th><th style={{ color: tColor }}>الحالة</th><th style={{ color: tColor }}>التاريخ</th></tr></thead><tbody>{invoicesData.map(inv=>(<TableRowWithHover key={inv.id} color={c}><td style={{ fontFamily: 'monospace', fontWeight: 800, color: tColor }}>{inv.number}</td><td style={{ fontWeight: 700 }}>{inv.customer}</td><td style={{ fontWeight: 800 }}>{formatCurrency(inv.total)}</td><td style={{ color: '#15803d', fontWeight: 700 }}>{formatCurrency(inv.paid)}</td><td style={{ color: inv.remaining>0?'#dc2626':'#64748b', fontWeight: 700 }}>{formatCurrency(inv.remaining)}</td><td><StatusBadge status={inv.status==='paid'?'تم الدفع':inv.status==='partially_paid'?'مدفوع جزئياً':'مؤكد'} /></td><td>{inv.date}</td></TableRowWithHover>))}</tbody></table></div></div></div>)}
      {activeTab === 'customers' && (<div className="space-y-6"><div className="grid grid-cols-4 gap-4">{[{label:'إجمالي العملاء',value:customersData.length,color:c},{label:'نشطين',value:customersData.filter(c=>c.status==='نشط').length,color:'#16a34a'},{label:'إجمالي الطلبات',value:customersData.reduce((s,c)=>s+c.totalOrders,0),color:'#2563eb'},{label:'إجمالي المبيعات',value:formatCurrency(customersData.reduce((s,c)=>s+c.totalSpent,0)),color:'#d97706'}].map((st,i)=>(<div key={i} className="stat-card" style={{ borderTop: `3px solid ${st.color}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, fontSize: '0.85rem', color: typeof st.value==='string'? '#64748b' : st.color as string }}>{st.label}</p><p className="stat-value">{st.value}</p></div><div className="stat-icon" style={{ background: `${st.color}15`, color: st.color as string }}><Users size={22} /></div></div></div>))}</div><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Users size={20} /></span>العملاء - {section.name}</h2><AddButton label="عميل جديد" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: tColor }}>العميل</th><th style={{ color: tColor }}>التواصل</th><th style={{ color: tColor }}>الطلبات</th><th style={{ color: tColor }}>الشراء</th><th style={{ color: tColor }}>آخر طلب</th><th style={{ color: tColor }}>الحالة</th></tr></thead><tbody>{customersData.map(cust=>(<TableRowWithHover key={cust.id} color={c}><td><div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}><div style={{ width: '2.5rem', height: '2.5rem', borderRadius: '0.65rem', background: `linear-gradient(135deg, ${tLight}, ${tColor})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900 }}>{cust.name[0]}</div><span style={{ fontWeight: 800 }}>{cust.name}</span></div></td><td><div style={{ display: 'flex', flexDirection: 'column' }}><span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.8rem' }}><Phone size={12} /> {cust.phone}</span><span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.8rem', color: '#64748b' }}><Mail size={12} /> {cust.email}</span></div></td><td style={{ fontWeight: 800, color: tColor }}>{cust.totalOrders}</td><td style={{ fontWeight: 800 }}>{formatCurrency(cust.totalSpent)}</td><td>{cust.lastOrderDate}</td><td><StatusBadge status={cust.status} /></td></TableRowWithHover>))}</tbody></table></div></div></div>)}
      {activeTab === 'debts' && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>إجمالي المديونيات</p><p className="stat-value" style={{ color: tColor }}>{formatCurrency(debtsData.reduce((s,d)=>s+d.amount,0))}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><CreditCard size={22} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><CreditCard size={20} /></span>المديونيات - {section.name}</h2><AddButton label="إضافة مديونية" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: tColor }}>النوع</th><th style={{ color: tColor }}>الاسم</th><th style={{ color: tColor }}>المبلغ</th><th style={{ color: tColor }}>تاريخ الاستحقاق</th><th style={{ color: tColor }}>الحالة</th><th style={{ color: tColor }}>الهاتف</th></tr></thead><tbody>{debtsData.map(d=>(<TableRowWithHover key={d.id} color={c}><td><span style={{ padding: '0.2rem 0.6rem', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem', background: d.type==='customer'?'#dbeafe':'#f3e8ff', color: d.type==='customer'?'#1d4ed8':'#7c3aed' }}>{d.type==='customer'?'عميل':'مورد'}</span></td><td style={{ fontWeight: 700 }}>{d.name}</td><td style={{ fontWeight: 800, color: tColor }}>{formatCurrency(d.amount)}</td><td>{d.dueDate}</td><td><StatusBadge status={d.status} /></td><td>{d.phone||'-'}</td></TableRowWithHover>))}</tbody></table></div></div>
        </div>
      )}
      {activeTab === 'expenses' && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>إجمالي المصروفات</p><p className="stat-value" style={{ color: tColor }}>{formatCurrency(expensesData.reduce((s,e)=>s+e.amount,0))}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><Receipt size={22} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Receipt size={20} /></span>المصروفات - {section.name}</h2><AddButton label="إضافة مصروف" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: tColor }}>التصنيف</th><th style={{ color: tColor }}>الوصف</th><th style={{ color: tColor }}>المبلغ</th><th style={{ color: tColor }}>التاريخ</th><th style={{ color: tColor }}>طريقة الدفع</th><th style={{ color: tColor }}>الحالة</th></tr></thead><tbody>{expensesData.map(e=>(<TableRowWithHover key={e.id} color={c}><td><span style={{ padding: '0.2rem 0.6rem', borderRadius: '0.5rem', fontWeight: 800, fontSize: '0.75rem', background: `${tColor}12`, color: tColor }}>{e.category}</span></td><td style={{ fontWeight: 700 }}>{e.description}</td><td style={{ fontWeight: 800, color: tColor }}>{formatCurrency(e.amount)}</td><td>{e.date}</td><td>{e.method}</td><td><StatusBadge status={e.status} /></td></TableRowWithHover>))}</tbody></table></div></div>
        </div>
      )}
      {activeTab === 'reports' && (<div className="space-y-6"><div className="grid grid-cols-3 gap-4"><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}`, padding: '1.5rem' }}><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><TrendingUp size={22} /></div><div><h3 style={{ fontWeight: 900, color: tColor, margin: 0 }}>نمو المبيعات</h3><p style={{ fontWeight: 700, color: '#64748b', fontSize: '0.85rem', margin: 0 }}>آخر 6 أشهر - {section.name}</p></div></div><div style={{ display: 'flex', alignItems: 'end', gap: '0.5rem', height: '100px', marginTop: '1rem' }}>{[40,65,55,80,70,95].map((h,i)=>(<div key={i} style={{ flex:1, background: `linear-gradient(to top, ${tColor}, ${tLight})`, height: `${h}%`, borderRadius: '0.5rem 0.5rem 0 0' }} />))}</div></div><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}`, padding: '1.5rem' }}><div className="flex items-center gap-3 mb-4"><div className="w-12 h-12 rounded-xl bg-gradient-to-br from-green-500 to-emerald-600 flex items-center justify-center text-white shadow-lg"><DollarSign size={22} /></div><div><h3 style={{ fontWeight: 900, color: '#059669', margin: 0 }}>الأرباح</h3><p style={{ fontWeight: 700, color: '#64748b', fontSize: '0.85rem', margin: 0 }}>هذا الشهر</p></div></div><p style={{ fontWeight: 900, fontSize: '2rem', color: '#059669' }}>{formatCurrency(125000)}</p></div><div className="card" style={{ border: `2px solid #f59e0b30`, borderTop: `4px solid #f59e0b`, padding: '1.5rem' }}><div className="flex items-center gap-3"><div className="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center text-white shadow-lg"><PackageCheck size={22} /></div><div><h3 style={{ fontWeight: 900, color: '#d97706', margin: 0 }}>كفاءة التصنيع</h3><p style={{ fontWeight: 700, color: '#64748b', fontSize: '0.85rem', margin: 0 }}>{recentProduction.filter(p=>p.status==='مكتمل').length} مكتمل / {recentProduction.length} إجمالي</p></div></div></div></div><div className="two-col-layout"><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header"><h2 style={{ fontWeight: 900, color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><BarChart3 size={20} /> تقرير المبيعات - {section.name}</h2></div><div className="card-body"><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}><div style={{ padding: '1rem', background: `${tColor}08`, borderRadius: '0.85rem' }}><p style={{ fontWeight: 700, color: '#64748b', fontSize: '0.8rem' }}>مبيعات اليوم</p><p style={{ fontWeight: 900, fontSize: '1.5rem', color: tColor }}>{formatCurrency(salesStats.todaySales)}</p></div><div style={{ padding: '1rem', background: '#f0fdf4', borderRadius: '0.85rem' }}><p style={{ fontWeight: 700, color: '#64748b', fontSize: '0.8rem' }}>الربح المتوقع</p><p style={{ fontWeight: 900, fontSize: '1.5rem', color: '#16a34a' }}>{formatCurrency(salesStats.todaySales*0.35)}</p></div></div></div></div><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header"><h2 style={{ fontWeight: 900, color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><Warehouse size={20} /> تقرير المخزون - {section.name}</h2></div><div className="card-body"><div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.75rem', background: `${tColor}08`, borderRadius: '0.75rem' }}><span>إجمالي القيمة</span><span style={{ fontWeight: 900, color: tColor }}>{formatCurrency(inventoryValue)}</span></div></div></div></div></div>)}
      {activeTab === 'academy' && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>الدورات المتاحة</p><p className="stat-value" style={{ color: tColor }}>{academyData.length}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><GraduationCap size={22} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><GraduationCap size={20} /></span>الأكاديمية - {section.name}</h2></div><div className="card-body p-0">{academyData.map(course=>(<div key={course.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1.25rem', borderBottom: `1px solid ${tColor}10` }}><div style={{ width: '3.5rem', height: '3.5rem', borderRadius: '0.85rem', background: `linear-gradient(135deg, ${tLight}, ${tColor})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}><PlayCircle size={24} /></div><div style={{ flex: 1 }}><h4 style={{ fontWeight: 800, color: '#1e293b', margin: 0 }}>{course.title}</h4><p style={{ fontWeight: 600, color: '#64748b', fontSize: '0.85rem', margin: 0 }}>{course.instructor} • {course.duration} • {course.level}</p><div style={{ marginTop: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}><div style={{ flex: 1, maxWidth: '200px', height: '0.5rem', background: `${tColor}15`, borderRadius: '999px', overflow: 'hidden' }}><div style={{ width: `${course.progress}%`, height: '100%', background: `linear-gradient(90deg, ${tColor}, ${tLight})` }} /></div><span style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>{course.progress}%</span></div></div><div style={{ textAlign: 'center' }}><p style={{ fontWeight: 800, color: tColor, margin: 0 }}>{course.enrolled}</p><p style={{ fontWeight: 600, color: '#64748b', fontSize: '0.75rem', margin: 0 }}>متدرب</p></div><StatusBadge status={course.status} /></div>))}</div></div>
        </div>
      )}
      {activeTab === 'books' && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="stat-card" style={{ borderTop: `3px solid ${tColor}` }}><div className="flex items-center justify-between"><div><p style={{ fontWeight: 800, color: tColor, fontSize: '0.85rem' }}>الكتب المتاحة</p><p className="stat-value" style={{ color: tColor }}>{booksData.length}</p></div><div className="stat-icon" style={{ background: `${tColor}15`, color: tColor }}><BookOpen size={22} /></div></div></div>
          ...
          <div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.35rem', color: tColor, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><BookOpen size={20} /></span>الكتب الرقمية - {section.name}</h2><AddButton label="كتاب جديد" color={c} colorLight={cl} /></div><div className="card-body p-0">{booksData.map(book=>(<div key={book.id} style={{ display: 'flex', alignItems: 'center', gap: '1.25rem', padding: '1.25rem', borderBottom: `1px solid ${tColor}10` }}><div style={{ width: '3.5rem', height: '3.5rem', borderRadius: '0.75rem', background: `linear-gradient(135deg, ${tLight}, ${tColor})`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '1.5rem', fontWeight: 900 }}>{book.title[0]}</div><div style={{ flex: 1 }}><h4 style={{ fontWeight: 800, color: '#1e293b', margin: 0 }}>{book.title}</h4><p style={{ fontWeight: 600, color: '#64748b', fontSize: '0.85rem', margin: '0.2rem 0 0 0' }}>{book.author} • {book.category} • {book.pages} صفحة</p></div><div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontWeight: 800, color: '#d97706' }}><Star size={16} /> {book.rating}</div><StatusBadge status={book.status} /></div>))}</div></div>
        </div>
      )}
      {activeTab === 'settings' && (<div className="space-y-6"><div className="card" style={{ border: `2px solid ${tColor}30`, borderTop: `4px solid ${tColor}` }}><div className="card-header" style={{ background: `linear-gradient(to bottom, ${tColor}08, rgba(248,250,252,0.9))` }}><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: tColor }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${tLight}, ${tColor})` }}><Settings size={24} /></span>إعدادات قسم {section.name}</h2></div><div className="card-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}><div style={{ gridColumn: 'span 2 / span 2' }}><h3 style={{ fontWeight: 800, fontSize: '1.1rem', color: tColor, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Palette size={18} /> المظهر والهوية</h3><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', padding: '1.25rem', background: `${tColor}06`, borderRadius: '1rem', border: `1.5px solid ${tColor}12` }}><div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>اسم القسم</label><input defaultValue={section.name} style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 700 }} /></div><div><label style={{ fontWeight: 700, fontSize: '0.85rem', color: '#475569', display: 'block', marginBottom: '0.4rem' }}>الوصف</label><input defaultValue={section.description} style={{ width: '100%', padding: '0.7rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${tColor}20`, fontWeight: 600 }} /></div></div></div><div><h3 style={{ fontWeight: 800, fontSize: '1.1rem', color: tColor, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Bell size={18} /> تنبيهات المخزون</h3><div style={{ padding: '1.25rem', background: '#fef2f2', borderRadius: '1rem', border: '1.5px solid #fecaca' }}><label style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontWeight: 700, color: '#7f1d1d' }}>تنبيه انخفاض المخزون</span><input type="checkbox" defaultChecked /></label></div></div><div><h3 style={{ fontWeight: 800, fontSize: '1.1rem', color: tColor, marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Shield size={18} /> الأمان</h3><div style={{ padding: '1.25rem', background: '#f0fdf4', borderRadius: '1rem', border: '1.5px solid #bbf7d0' }}><label style={{ display: 'flex', justifyContent: 'space-between' }}><span style={{ fontWeight: 700, color: '#14532d' }}>تأكيد قبل الحذف</span><input type="checkbox" defaultChecked /></label></div></div><div style={{ gridColumn: 'span 2 / span 2', display: 'flex', justifyContent: 'end', marginTop: '0.5rem' }}><div style={{ padding: '0.75rem 1.75rem', borderRadius: '0.85rem', background: `linear-gradient(135deg, ${tLight}, ${tColor})`, color: '#fff', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Save size={18} /> حفظ الإعدادات</div></div></div></div></div>)}
    </div>
  );
}
