...
<div className="card animate-slide-up" style={{ animationDelay: '0.5s', animationFillMode: 'both', border: '2px solid #e2e8f0', background: 'rgba(255,255,255,0.7)' }}><div className="card-header"><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: c }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><ShoppingCart size={24} /></span>إجراءات سريعة</h2></div><div className="card-body"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '1.25rem' }}>{quickActions.map((action, index) => (<QuickActionLink key={action.label} action={action} index={index} sectionColor={c} />))}</div></div></div>
          <div className="two-col-layout">
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.7s', border: `2px solid #7c3aed30`, borderTop: `4px solid #7c3aed` }}><div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, #7c3aed08, rgba(248,250,252,0.9))` }}><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#7c3aed' }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, #a78bfa, #7c3aed)` }}><Beaker size={24} /></span>آخر التركيبات - {section.name}</h2><SectionLink label="عرض الكل" color={c} onClick={() => router.push(`${pathname}?tab=formulas`)} /></div><div className="card-body p-0">{formulas.slice(0, 4).map((f, i) => (<ListItem key={i} name={f.name} subtitle={`${f.category} • ${f.ingredients} مكون`} badge={f.status} emoji={formulaEmojis[f.name] || '🧪'} color={c} />))}</div></div>
            <div className="card list-card animate-slide-up" style={{ animationDelay: '0.8s', border: `2px solid #16a34a30`, borderTop: `4px solid #16a34a` }}><div className="card-header flex items-center justify-between" style={{ background: `linear-gradient(to bottom, #16a34a08, rgba(248,250,252,0.9))` }}><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#16a34a' }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, #4ade80, #16a34a)` }}><Package size={24} /></span>آخر المنتجات - {section.name}</h2><SectionLink label="عرض الكل" color={c} onClick={() => router.push(`${pathname}?tab=products`)} /></div><div className="card-body p-0">{products.slice(0, 4).map((p, i) => (<ListItem key={i} name={p.name} subtitle={`${p.category} • مخزون: ${p.stock}`} value={formatCurrency(p.price)} emoji="📦" color={c} />))}</div></div>
          </div>
