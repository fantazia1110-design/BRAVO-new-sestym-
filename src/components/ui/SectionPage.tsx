...
<div className="card animate-slide-up" style={{ animationDelay: '0.5s', animationFillMode: 'both', border: '2px solid #e2e8f0', background: 'rgba(255,255,255,0.7)' }}><div className="card-header"><h2 style={{ fontWeight: 900, fontSize: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', color: c }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><ShoppingCart size={24} /></span>إجراءات سريعة</h2></div><div className="card-body"><div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '1.25rem' }}>{quickActions.map((action, index) => (<QuickActionLink key={action.label} action={action} index={index} sectionColor={c} />))}</div></div></div>
      ...
      {activeTab === 'production' && (<div className="card" style={{ border: `2px solid ${c}30`, borderTop: `4px solid ${c}` }}><div className="card-header flex items-center justify-between"><h2 style={{ fontWeight: 900, fontSize: '1.5rem', color: c, display: 'flex', alignItems: 'center', gap: '0.75rem' }}><span className="w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg" style={{ background: `linear-gradient(135deg, ${cl}, ${c})` }}><Factory size={24} /></span>التصنيع ({recentProduction.length}) - {section.name}</h2><AddButton label="بدء تصنيع" color={c} colorLight={cl} /></div><div className="card-body p-0"><table className="table"><thead><tr><th style={{ color: c }}>رقم الدفعة</th><th style={{ color: c }}>المنتج</th><th style={{ color: c }}>الكمية</th><th style={{ color: c }}>التقدم</th><th style={{ color: c }}>الحالة</th></tr></thead><tbody>{recentProduction.map((b, i) => (<TableRowWithHover key={i} color={c}><td style={{ fontWeight: 800, fontFamily: 'monospace', color: c }}>{b.batch}</td><td style={{ fontWeight: 700 }}>{b.product}</td><td>{b.qty}</td><td><ProgressBar progress={b.progress} color={c} colorLight={cl} /></td><td><StatusBadge status={b.status} /></td></TableRowWithHover>))}</tbody></table></div></div>)}
